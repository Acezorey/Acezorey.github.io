---
layout: project
type: project
image: img/deeplearning.jpg
title: "Image Recognition Neural Network"
date: 2025-01
published: true
labels:
  - Artificial Intelligence
  - Python
  - NumPy
  - Educational
  - Jupyter Notebook
summary: "Neural network developed and trained to recognize 2D images"
---

<img width="1000px" src="../img/dataset-cover.png">

## Overview
This program is a deep learning neural network developed entirely without using any machine learning libraries such as PyTorch, using only pure Python and NumPy. Designed with guidance from the book <a href="https://nnfs.io/">Neural Networks from Scratch</a>, this was made for educational purposes to learn of the mathematics and mechanics behind deep learning models.

## Technical Details
This model was trained on the Fashion MNIST dataset (accessable on <a href="https://www.kaggle.com/datasets/zalando-research/fashionmnist">Kaggle</a>), a dataset containing tens of thousands of labeled 28x28 pixel images of articles of clothing.

<img width="700px" src="../img/modeltraining.png">

It is also indeed capable of recognizing pieces of clothing from test images, including hand drawn ones.

<img width="700px" src="../img/modelprediction.png">

## Links
Github: <a href="https://github.com/Acezorey/neural-network-learning">Link</a>
